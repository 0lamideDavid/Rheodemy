...
export default function DashboardLayout({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    const isCreatorPath = pathname?.includes('/dashboard/creator');
    const isLearnerPath = pathname?.includes('/dashboard/learner');

    if (isCreatorPath && user.role === 'LEARNER') {
      setAccessDeniedMessage("You don't have access to this page.");
      router.push('/dashboard/learner');
    } else if (isLearnerPath && user.role === 'CREATOR') {
      setAccessDeniedMessage("You don't have access to this page.");
    ...
    <div className="min-h-screen bg-background flex flex-col relative overflow-hidden">
      ...
      {accessDeniedMessage && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-md animate-in fade-in duration-200">
          <div className="glass max-w-sm w-full p-8 rounded-3xl border border-white/10 relative overflow-hidden shadow-2xl text-center space-y-6">
            ...
            <div className="space-y-2">
              ...
              <p className="text-sm text-foreground/70 leading-relaxed">{accessDeniedMessage}</p>
            </div>
            <button
              onClick={() => setAccessDeniedMessage(null)}
              className="w-full bg-foreground text-background font-bold py-3 px-4 rounded-xl hover:opacity-90 active:scale-95 transition-all shadow-lg"
            >
              Acknowledge
            </button>
          </div>
